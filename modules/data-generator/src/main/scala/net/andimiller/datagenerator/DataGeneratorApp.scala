...
class DataGeneratorApp[T: Arbitrary: Encoder](programName: String) extends IOApp {
  ...
  def items[F[_]: Monad: Async: Random]: Stream[F, String] =
    for {
  ...
  def generate[F[_]: Monad: Async: Random](n: Long): F[Unit] =
    items[F]
      .take(n)
  ...
  def stream[F[_]: Monad: Async: Random](every: FiniteDuration): F[Unit] =
    Stream
      .awakeEvery(every)
      .zip(items[F])
      .map(_._2)
